...
import { CourseVM } from 'src/app/views/lms/Models/CourseVM';
import { LMSService } from 'src/app/views/lms/lms.service';

@Component({
  selector: 'app-manage-courseschedule',
  templateUrl: './manage-courseschedule.component.html',
  styleUrls: ['./manage-courseschedule.component.css']
})
export class ManageCoursescheduleComponent implements OnInit {
  ...
  WeekDays?: SettingsVM[]
  ScheduleId!: number
  getSchById!: CourseScheduleVM[];
  Schedules: CourseScheduleVM[] = []
  selectedSchedule = new CourseScheduleVM
  ...
  constructor(
    ...
    this.selectedSchedule = new CourseScheduleVM();
  ...
  GetWeekDays() {
    ...
    stng.enumTypeId = EnumTypes.WeekDays //entities,scheduletype nd working type
  ...
  GetCourseScheduleById() {
    debugger
    this.schSvc.GetCourseScheduleById(this.ScheduleId).subscribe({
      next: (res: CourseScheduleVM[]) => {
        debugger
        this.getSchById = res;
  ...
  SaveSchedule() {
    ...
    this.schSvc.SaveCourseSchedule(this.selectedSchedule).subscribe({
      next: (res: CourseScheduleVM) => {
  ...
  UpdateSchedule() {
    this.schSvc.UpdateCourseSchedule(this.selectedSchedule).subscribe({
      next: (res: CourseScheduleVM) => {
        if (this.Edit) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          if (ScheduleDet.id == undefined || ScheduleDet.id == 0) {
            ...
            var Schedule = new CourseScheduleVM
            Schedule = this.selectedSchedule
            Schedule.classTimings = []
            Schedule.classTimings.push(ScheduleDet)
            ScheduleDet.dBoperation = 3
            this.schSvc.UpdateCourseSchedule(Schedule).subscribe({
              next: (data: CourseScheduleVM) => {
                Swal.fire(
                  'Deleted!',
                  'Successfully Deleted.',
                  'success'
