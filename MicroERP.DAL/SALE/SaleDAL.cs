        ...
        public List<SaleDE> SearchSale(string whereClause, MySqlCommand cmd = null)
        {
            ...
            try
            {
                ...
                if (cmd.Connection.State == ConnectionState.Open)
                    ...
                    Console.WriteLine("Connection error");
                top = cmd.Connection.Query<SaleDE>("call MicroERP.SearchSale( '" + whereClause + "')").ToList();
                return top;
            }
            catch (Exception)
            {
        ...
        public List<SaleLineDE> SearchSaleLine(string whereClause, MySqlCommand cmd = null)
        {
            ...
            try
            {
                ...
                if (cmd.Connection.State == ConnectionState.Open)
                    ...
                    Console.WriteLine("Connection error");
                top = cmd.Connection.Query<SaleLineDE>("call MicroERP.SearchSaleLine( '" + whereClause + "')").ToList();
                return top;
            }
            catch (Exception)
            {
