        ...
        public List<CourseScheduleDE> SearchCourseSchedules(string whereClause, MySqlCommand cmd = null)
        {
            ...
            try
            {
                ...
                if (cmd.Connection.State == ConnectionState.Open)
                    ...
                    Console.WriteLine("Connection error");
                top = cmd.Connection.Query<CourseScheduleDE>("call MicroERP.SearchCourseSchedule( '" + whereClause + "')").ToList();
                return top;
            }
            catch (Exception)
            {
        ...
        public List<ClassTimingDE> SearchClassTimings(string whereClause, MySqlCommand cmd = null)
        {
            ...
            try
            {
                ...
                if (cmd.Connection.State == ConnectionState.Open)
                    ...
                    Console.WriteLine("Connection error");
                top = cmd.Connection.Query<ClassTimingDE>("call MicroERP.SearchClassTiming( '" + whereClause + "')").ToList();
                return top;
            }
            catch (Exception exp)
            {
