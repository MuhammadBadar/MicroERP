    ...
    public class AppointmentDAL :  IBaseDAL<AppointmentDE>, IApptDAL
    {
        ...
        public List<AppointmentDE> SearchData ( string whereClause, MySqlCommand? _cmd )
        {
            ...
            try
            {
                ...
                appts = _cmd.Connection.Query<AppointmentDE> ("call MicroERP.SearchAppointment('" + whereClause + "')").ToList ();
                return appts;
            }
            catch (Exception)
            {
        ...
        public List<AppointmentDE> SearchNextAppt ( string whereClause,int ClientId,int DoctorId, MySqlCommand? _cmd )
        {
            ...
            try
            {
                ...
                string procedureName = "MicroERP.GetNextAppt";
                string sql = $"CALL {procedureName}(@whereClause, @clientId, @doctorId)";
                //appts = _cmd.Connection.Query<AppointmentDE> ("call MicroERP.GetNextAppt('" + whereClause + ""+","+"" + ClientId + " ')").ToList (); 
        ...
        public List<AppointmentDE> SearchAppointment ( string WhereClause, MySqlCommand cmd )
        {
            ...
            try
            {
                ...
                test = cmd.Connection.Query<AppointmentDE> ("call MicroERP.SearchAppointment('" + WhereClause + "')").ToList ();
                return test;
            }
            catch (Exception)
            {
        ...
        public List<AppointmentDE> SearchNextAppmt ( string WhereClause, MySqlCommand cmd )
        {
            ...
            try
            {
                ...
                test = cmd.Connection.Query<AppointmentDE> ("call MicroERP.GetNextAppt('" + WhereClause + "')").ToList ();
                return test;
            }
            catch (Exception)
            {
