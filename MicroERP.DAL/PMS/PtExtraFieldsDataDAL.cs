        ...
        public List<PtExtraFieldsDataDE> SearchPtExtraFieldsData ( string whereClause, MySqlCommand cmd = null )
        {
            ...
            try
            {
                ...
                if (cmd.Connection.State == ConnectionState.Open)
                    ...
                    Console.WriteLine ("Connection error");
                top = cmd.Connection.Query<PtExtraFieldsDataDE> ("call MicroERP.SearchPtExtraFieldsData( '" + whereClause + "')").ToList ();
                return top;
            }
            catch (Exception)
            {
        ...
        public List<Object> GetPtExtraFieldsData ( int TypeId, MySqlCommand cmd = null )
        {
            ...
            try
            {
                ...
                if (cmd.Connection.State == ConnectionState.Open)
                    ...
                    Console.WriteLine ("Connection error");
                ds = cmd.Connection.Query<Object> ("call MicroERP.GetExtraFieldsData( " + TypeId + ")").ToList ();
                return ds;
            }
            catch (Exception)
            {
