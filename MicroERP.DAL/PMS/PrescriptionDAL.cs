        ...
        public List<PrescriptionDE> SearchPrescriptions ( string whereClause, MySqlCommand cmd = null )
        {
            ...
            try
            {
                ...
                if (cmd.Connection.State == ConnectionState.Open)
                    ...
                    Console.WriteLine ("Connection error");
                top = cmd.Connection.Query<PrescriptionDE> ("call MicroERP.SearchPrescription( '" + whereClause + "')").ToList ();
                return top;
            }
            catch (Exception exp)
            {
        ...
        public List<RxMedicineDE> SearchRxMedicine ( string whereClause, MySqlCommand cmd = null )
        {
            ...
            try
            {
                ...
                if (cmd.Connection.State == ConnectionState.Open)
                    ...
                    Console.WriteLine ("Connection error");
                top = cmd.Connection.Query<RxMedicineDE> ("call MicroERP.SearchRxMedicine( '" + whereClause + "')").ToList ();
                return top;
            }
            catch (Exception exp)
            {
